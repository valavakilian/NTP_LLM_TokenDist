    ...
    for X in tqdm(dataloader):
        ...
        if contexts_count <= up_to_ctx_count_processed: 
            ...
            if milestone_index < len(milestones) and contexts_count >= milestones[milestone_index]:
                ...
                plot_entropy_perCtxLen(data_log_MT, save_graph_folder + f"entropy_voc_{args.vocab_size}_ctxLen_{args.context_length}_stride{args.stride}_{args.perc_stories}%TS.jpg", precDone = round(batches_seen / len(dataloader) * 100), ctx_len = [t for t in range(0, args.root_ctx_len)])
