...
def load_and_tokenize_wikitext(dataset_dir, vocab_size, context_length):
    ...
    merged_text = merged_text[0:len(merged_text)//20]
...
def plot_data_log_subplots(data_log, file_path, precDone):
    ...
    entropy_withMem = [data_log['entropy_withMem'][doc] for doc in doc_counts]
    ...
    axs[0].plot(doc_counts, entropy_withMem, label='Entropy Old withMem', marker='o', color = "green")
...
def plot_calc_times(data_log, file_path, precDone):
    ...
    entropy_calc_time_withMem = [data_log['entropy_calc_time_withMem'][doc] for doc in doc_counts]
    ...
    plt.plot(doc_counts, entropy_calc_time_withMem, label='Entropy Calculation Time Old withMem', marker='o', color = "purple")
...
if __name__ == "__main__":
    ...
    try:
        ...
        context_tree_old = trie_module_memap_sorted_old.Trie_memap_sorted_old(memap_filename + "_old.bin", memap_filename + "_metadata_old.bin", 100, args.context_length)
        context_tree = trie_module_memap_sorted.Trie_memap_sorted(memap_filename, 100, args.context_length)
        ...
        for X in dataloader:
            ...
            if contexts_count <= up_to_ctx_count_processed: 
                ...
                if milestone_index < len(milestones) and contexts_count >= milestones[milestone_index]:
                    ...
                    print("Took " + str(time.time() - start_time_entropy) + " sec.")
                    data_log["entropy_withMem"][contexts_count] = entropy_tree
