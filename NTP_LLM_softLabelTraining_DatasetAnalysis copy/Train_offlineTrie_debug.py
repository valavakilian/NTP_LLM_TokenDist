...
if __name__ == "__main__":
    ...
    args.step_size = args.context_length // 8
    num_epochs = 90
    ...
    optimizer_one_hot = AdamW(model_one_hot.parameters(), lr=5e-2)
    optimizer_soft_label = AdamW(model_soft_label.parameters(), lr=5e-2)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(dataloader):  
            ...
            for i in range(0, batch.shape[0]):
                for ctx_idx in range(0, y_soft_label.shape[1]):
                    norm_soft_vs_hard_diff += 2 * (1 - y_soft_label[i][ctx_idx][y_one_hot[i,ctx_idx].item()])
