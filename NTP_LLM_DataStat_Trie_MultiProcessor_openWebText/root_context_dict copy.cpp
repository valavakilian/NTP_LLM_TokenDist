...
private:
    ...
    void process_chunk(const int64_t* data, size_t start_idx, size_t end_idx, size_t seq_length) {
        // Create thread-local maps to avoid constant locking
        std::unordered_map<uint16_t, std::unordered_map<uint16_t, uint32_t>> local_single_counts;
        std::unordered_map<std::pair<uint16_t, uint16_t>, std::unordered_map<uint16_t, uint32_t>, PairHash> local_pair_counts;
        ...
        for (size_t i = start_idx; i < end_idx; i++) {
            ...
            if (context_length == 1) {
                ...
                local_single_counts[context][next_token]++;
            } else {
                ...
                local_pair_counts[context][next_token]++;
            }
        }
        ...
        if (context_length == 1) {
            for (const auto& [ctx, dist] : local_single_counts) {
                auto& global_dist = single_context_counts[ctx];
                for (const auto& [token, count] : dist) {
                    global_dist[token] += count;
                }
            }
        } else {
            for (const auto& [ctx, dist] : local_pair_counts) {
                auto& global_dist = pair_context_counts[ctx];
                for (const auto& [token, count] : dist) {
                    global_dist[token] += count;
                }
            }
...
public:
    ...
    InsertResult insert(py::array_t<int64_t, py::array::c_style> batch, bool verbose = false) {
        ...
        for (unsigned int i = 0; i < num_threads; i++) {
            ...
            threads.emplace_back(&TokenDistributionCounter::process_chunk, this, 
