...
struct RAMTrieNode {
    ...
    std::pair<uint16_t, int64_t>* children;  // value is uint16_t since max token is 50K
...
struct MMAPTrieNode {
    ...
    int64_t children_offset;
    uint16_t node_level;
    uint16_t value;  // matches the uint16_t in children pair
};
...
private:
    ...
    uint16_t find_child(RAMTrieNode* node, uint16_t value) {
    ...
    void insert_child(RAMTrieNode* node, uint16_t value, int64_t child_index) {
...
public:
    ...
    std::vector<std::unordered_map<uint16_t, double>> insert_context(const torch::Tensor& tensor, int64_t column, bool return_prob_distr) {   
        std::vector<std::unordered_map<uint16_t, double>> distributions;
        ...
        for (int64_t j = 0; j < accessor.size(1); j++) {
            uint16_t value = accessor[column][j];
            ...
            if (return_prob_distr) {
                // Create distribution
                std::unordered_map<uint16_t, double> current_distribution;
    ...
    InsertResult insert(torch::Tensor tensor, bool return_prob_distr) {
        ...
        std::vector<std::vector<std::unordered_map<uint16_t, double>>> soft_label_distributions(batch_size);
    ...
    std::vector<std::unordered_map<uint16_t, double>> retrieve_context_softlabel(const torch::Tensor& tensor, int64_t column) {
        if (is_construction_mode) {
        ...
        for (int64_t j = 0; j < accessor.size(1); j++) {
            ...
            if (current->num_children > 0) {
                ...
                uint16_t child_index = find_child(children, current->num_children, value);
                ...
                if (child_index != -1) {
                    ...
                    std::unordered_map<uint16_t, double> distribution;
                    if (current->num_children > 0) {
    ...
    py::list retrieve_softlabel(const torch::Tensor& tensor) {
        ...
        std::vector<std::vector<std::unordered_map<uint16_t, double>>> soft_label_distributions(tensor.size(0));
...
py::dict convert_to_python_dict(const std::unordered_map<std::vector<uint16_t>, int64_t>& sequences) {
    py::dict result;
