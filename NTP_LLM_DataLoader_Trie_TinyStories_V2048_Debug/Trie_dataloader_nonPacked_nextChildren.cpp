...
private:
    ...
    std::pair<uint16_t, int64_t>* get_mmap_children(MMAPTrieNode* node) {
        ...
        // Calculate the pointer to the children array
            ...
            if (nodes[i].num_children > 0) {
                ...
                for (uint16_t j = 0; j < nodes[i].num_children; j++) {
                    child_pairs[j] = {
                        ...
                        static_cast<int64_t>(node_addresses[nodes[i].children[j].second])
                    };
                }
                memcpy(mapped_memory + children_addresses[i], child_pairs.data(),
