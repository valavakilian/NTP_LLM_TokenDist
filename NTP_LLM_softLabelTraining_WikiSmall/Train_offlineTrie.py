...
class WikitextShiftedDataset(Dataset):
    def __init__(self, tokenized_data, context_length):
        ...
        self.step_size = context_length // 2  # Shift by half the context length
...
if __name__ == "__main__":
    ...
    tokenized_data, tokenizer = load_and_tokenize_wikitext(dataset_dir, vocab_size, data_ratio = 0.2)

    # Step 5: Create the DataLoader
    dataloader, num_ctx = create_dataloader(tokenized_data, context_length, batch_size)

    num_milestones = 10
    memap_filename = "voc{args.vocab_size}_ctxLen{args.context_length}"
    ...
    #     output = context_tree.retrieve_softlabel(exe_of_ctx_to_get)
    #     print(exe_of_ctx_to_get)
    #     print(output)
    #     input()
    ...
    model_one_ont = GPT2LMHeadModel(model_config)
    model2 = GPT2LMHeadModel(model_config)

    # Move models to GPU if available
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model_one_ont.to(device)
    model2.to(device)


    # Optimizers for each model
    optimizer_one_hot = AdamW(model_one_ont.parameters(), lr=5e-5)
    optimizer2 = AdamW(model2.parameters(), lr=5e-5)

    # Optimizers for each scheduler
    scheduler_one_hot = torch.optim.lr_scheduler.StepLR(optimizer_one_hot, step_size=10, gamma=0.1)  # Reduce LR every 10 epochs by a factor of 0.1
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            full_sequence = batch.to(device)
            
            # Extract `x` (input) and `y` (target) from the full sequence
            x_batch = full_sequence[:, :-1]  # Everything except the last token
            y_batch = full_sequence[:, 1:]   # Everything except the first token
            
            inputs = torch.tensor(batch['input_ids']).to(device)
            labels = inputs.clone().to(device)

            # Forward pass for model 1
            outputs = model_one_ont(x_batch, labels=y_batch)
            loss = outputs.loss
        ...
        print(f"Epoch {epoch+1}/{num_epochs} - Loss Model 1: {total_loss_one_hot/len(dataloader1)}, Loss Model 2: {total_loss2/len(dataloader2)}")
