...
struct InsertResult_ST {
    py::list result;
    double execution_time_ms;
};


class Trie_module_protV1_ST {
private:
    int fd;
    char* mapped_memory;
...
public:
    Trie_module_protV1_ST(const std::string& fname, size_t initial_size_gb, int64_t context_length) 
    ...
    Trie_module_protV1_ST(const std::string& fname) : 
    ...
    std::vector<std::unordered_map<int64_t, double>> insert_context(const torch::Tensor& tensor, int64_t column, bool return_prob_distr) {   
        ...
        for (int64_t j = 0; j < accessor.size(1); j++) {
            ...
            curr_node_count = current->count;

            if (current->node_index > 0 && c_t_temp > 0 && current->node_level <= context_length){
                if(logcalc_memory_insert[c_t_temp] == -1){
                    logcalc_memory_insert[c_t_temp] = (c_t_temp + 1) * std::log(c_t_temp + 1) - (c_t_temp) * std::log(c_t_temp);
                }
                countLog_array[current->node_index] += logcalc_memory_insert[c_t_temp];
    ...
    InsertResult_ST insert(torch::Tensor tensor, bool return_prob_distr) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    py::class_<Trie_module_protV1_ST>(m, "Trie_module_protV1_ST")
        .def(py::init<const std::string&, size_t, int64_t>())
        .def(py::init<const std::string&>())
        .def("insert", &Trie_module_protV1_ST::insert)
        .def("retrieve_softlabel", &Trie_module_protV1_ST::retrieve_softlabel)
        .def("collect_all_sequences", [](Trie_module_protV1_ST& trie) {
            auto sequences = trie.collect_all_sequences();
            return convert_to_python_dict(sequences);
        })
        .def("get_memory_usage", &Trie_module_protV1_ST::get_memory_usage)
        .def("get_allocated_size", &Trie_module_protV1_ST::get_allocated_size)
        .def("get_num_unique_contexts", &Trie_module_protV1_ST::get_num_unique_contexts)  // New method to access num_unique_contexts
        .def("get_num_total_contexts", &Trie_module_protV1_ST::get_num_total_contexts)  // New method to access num_unique_contexts
        .def("get_children_distribution", &Trie_module_protV1_ST::get_children_distribution)
        .def("get_node_count", &Trie_module_protV1_ST::get_node_count)
        .def("get_node_level", &Trie_module_protV1_ST::get_node_level)
        .def("get_num_unique_contexts_per_level", &Trie_module_protV1_ST::get_num_unique_contexts_per_level)
        .def("get_num_total_contexts_per_level", &Trie_module_protV1_ST::get_num_total_contexts_per_level)
        .def("get_entropy_per_level", &Trie_module_protV1_ST::get_entropy_per_level)
        .def("calculate_and_get_entropy_faster", &Trie_module_protV1_ST::calculate_and_get_entropy_faster)
        .def("load_metadata", &Trie_module_protV1_ST::load_metadata)
        .def("save_metadata", &Trie_module_protV1_ST::save_metadata);


    py::class_<InsertResult_ST>(m, "InsertResult_ST")
        .def_readonly("result", &InsertResult_ST::result)
        .def_readonly("execution_time_ms", &InsertResult_ST::execution_time_ms);


    // py::class_<MemMapArray<int64_t>>(m, "MemMapArrayInt_ST")
    //     .def(py::init<const std::string&, size_t>())
    //     .def("get_size", &MemMapArray<int64_t>::getSize)
    //     .def("__getitem__", [](MemMapArray<int64_t> &self, size_t index) { return self[index]; });
    
    // py::class_<MemMapArray<double>>(m, "MemMapArrayDouble_ST")
    //     .def(py::init<const std::string&, size_t>())
