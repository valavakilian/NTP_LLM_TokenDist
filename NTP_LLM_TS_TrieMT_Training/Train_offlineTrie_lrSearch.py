...
# First, let's modify the criterion to handle both types of targets
class CustomCriterion:
    ...
    def __call__(self, logits, batch):
        ...
        loss = -(y * log_probs).sum() / (batch.shape[0] * batch.shape[1])
        return loss


def find_optimal_lrs(model_one_hot, model_soft_label, train_dataloader, device, args, context_tree):
    """
    Find optimal learning rates for both models
    """
    results = {}
    
    # Setup for one-hot model
    print("\nFinding optimal LR for one-hot model...")
    optimizer_one_hot = AdamW(model_one_hot.parameters(), lr=1e-7)
    criterion_one_hot = CustomCriterion('one_hot')
    
    lr_finder_one_hot = LRFinder(
        model=model_one_hot,
        optimizer=optimizer_one_hot,
        criterion=criterion_one_hot,
        device=device,
        min_lr=1e-7,
        max_lr=1,  # Reduced from 10 to 1 as it's rare to need lr > 1
        num_iterations=100
    )
    
    # Run LR finder for one-hot model
    history_one_hot = lr_finder_one_hot.range_test(train_dataloader)
    
    # Plot one-hot results
    plt.figure(figsize=(10, 6))
    lr_finder_one_hot.plot()
    plt.title("Learning Rate Finder Results - One Hot Model")
    plt.savefig('lr_finder_one_hot.png')
    plt.close()
    
    # Setup for soft-label model
    print("\nFinding optimal LR for soft-label model...")
    ...
    lr_finder_soft_label = LRFinder(
        ...
        criterion=criterion_soft_label,
        device=device,
        min_lr=1e-7,
        max_lr=1,
        num_iterations=100
    ...
    # Run LR finder for soft-label model
    history_soft_label = lr_finder_soft_label.range_test(train_dataloader)
    
    # Plot soft-label results
    plt.figure(figsize=(10, 6))
    lr_finder_soft_label.plot()
    plt.title("Learning Rate Finder Results - Soft Label Model")
    plt.savefig('lr_finder_soft_label.png')
    plt.close()
    
    # Find optimal learning rates using the steepest gradient method
    def get_optimal_lr(history):
        ...
        return lrs[steepest_point]
    
    optimal_lr_one_hot = get_optimal_lr(history_one_hot)
    optimal_lr_soft_label = get_optimal_lr(history_soft_label)
    
    return optimal_lr_one_hot, optimal_lr_soft_label
...
if __name__ == "__main__":
    ...
    optimal_lr_one_hot, optimal_lr_soft_label = find_optimal_lrs(
        model_one_hot,
        model_soft_label,
        dataloader,
        device,
    ...
    print(f"One-hot model: {optimal_lr_one_hot:.2e}")
    print(f"Soft-label model: {optimal_lr_soft_label:.2e}")
