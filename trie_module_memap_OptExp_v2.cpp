...
//         // Parallel for loop with reduction to sum across threads
//         #pragma omp parallel for reduction(+:total)
//         for (size_t i = 0; i < size; ++i) {
//             total += mapped_memory[i];
//         }
...
//         int file_size_value = file_size.load();
//         return (allocated_size - file_size_value) <= MEMORY_THRESHOLD;
//     }

//     bool is_close_to_counter_limit(int num_input_seq) const {
//         int node_mutex_counter_value = node_mutex_counter.load();
//         return (node_mutex_counter_value + num_input_seq * context_length) >= size_logcalc_memory;
//     }
...
//         if (is_close_to_counter_limit(tensor.size(0))) {
//             save_metadata();
//             return -1;  // Indicating we're running out of array space
//         }

//         const int num_threads = 64;  // Number of threads in the pool. Adjust as needed.
...
//         auto worker = [this, &tensor, &next_batch, &done, batch_size]() {
//             while (!done) {
...
//                     this->insert_context(tensor, col);
//                 }
//             }
//         };
...
//     //     #pragma omp parallel for private(entropy_temp) reduction(+:total_entropy, total_counter, counter)
//     //     for (int j = 1; j < node_counter; j++) {
//     //         entropy_temp = countLog_array[j] - ctxCount_array[j] * log(ctxCount_array[j]);
...
//     //         #pragma omp critical  // Protect shared map access
//     //         {
//     //             num_unique_contexts_per_level[ctxLen_array[j]] += 1;
//     //             entropy_per_level[ctxLen_array[j]] += entropy_temp;
//     //         }
...
//         int64_t parallel_total_sum = ctxCount_array.parallel_sum();
//         return parallel_total_sum;
//     }
...
public:
    ...
    //         if(logcalc_memory_entropy[j] == -1){
    //             logcalc_memory_entropy[j] = ctxCount_array[j] * log(ctxCount_array[j]);
    //         }
    //         entropy_temp = countLog_array[j] - logcalc_memory_entropy[j];
