...
public:
    ...
    ctxLen_array(fname + "_ctxLen_arr.dat", array_size), ctxCount_array(fname + "_ctxCount_arr.dat", array_size), softLabelDiff_array(fname + "_softLabelDiff_arr.dat", array_size)
    ...
    ctxLen_array(fname + "_ctxLen_arr.dat"), ctxCount_array(fname + "_ctxCount_arr.dat"), softLabelDiff_array(fname + "_softLabelDiff_arr.dat")
    , logcalc_memory_insert(size_logcalc_memory, -1), logcalc_memory_entropy(size_logcalc_memory, -1), mutex_array_lock(array_size){
    ...
    std::vector<std::unordered_map<int64_t, double>> insert_context(const torch::Tensor& tensor, int64_t column, bool return_prob_distr) {   
        ...
        for (int64_t j = 0; j < accessor.size(1); j++) {
            ...
            if (current->node_index > 0 && c_t_temp > 0 && current->node_level <= context_length){
                ...
                // Additionally, upadte the diff terms  
                softLabelDiff_array[current->node_index] += softLabelDiff_array[current->node_index] * (pow(curr_node_count-1, 2) / pow(curr_node_count, 2)) + (4/(pow(curr_node_count, 2))) * (curr_node_count - 1 - c_t_temp) ;
            }
    ...
    std::pair<double, double> calculate_and_get_entropy_faster() {
        ...
        DEBUG_PRINT("total_softLabelDiff: " << total_softLabelDiff);
        ...
        return std::make_pair(total_entropy , total_softLabelDiff);  // Normalize by total number of nodes
    }
