...
def load_or_create_tree(args, memap_filename, dataloader, num_milestones, num_examples):
    ...
    save_tree_folder =  "/scratch/st-cthrampo-1/vaalaa/NTP_LLM_TinyStories/Trie_CPP_saves_multithreaded_Debug/MT/context_trees_memap_cpp/"
    ...
    context_tree_ST = trie_module_protV1_lib.Trie_module_protV1(memap_filename_ST, 200, args.context_length)
    ...
    for X in tqdm(dataloader):
        ...
        if contexts_count <= up_to_ctx_count_processed: 
            ...
            _ = context_tree_MT.insert(X, False)
            # insert_runtime += time.time() - start_time_insert
            insert_runtime_MT += timer() - start_time_insert

            # start_time_insert = time.time()
            start_time_insert = timer()
            _ = context_tree_ST.insert(X, False)
            # insert_runtime += time.time() - start_time_insert
            ...
            if milestone_index < len(milestones) and contexts_count >= milestones[milestone_index]:
                ...
                data_log_MT["insert_calc_time"][contexts_count] = insert_runtime
                data_log_ST["insert_calc_time"][contexts_count] = insert_runtime
...
if __name__ == "__main__":
    ...
    context_tree_MT = load_or_create_tree(args, memap_filename, dataloader, num_milestones, num_ctx)
