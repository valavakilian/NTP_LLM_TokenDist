...
public:
    ...
    // Add this method to get the size of the thread pool
    size_t size() const {
        return workers.size();
    }
};
...
private:
    ...
    const size_t size_logcalc_memory = 10000000;  // 1 billion integers (~4 GB)
...
public:
    ...
    , logcalc_memory(size_logcalc_memory, -1), mutex_array_lock(array_size), thread_pool(16) {
    ...
    int insert(torch::Tensor tensor) {
        ...
        DEBUG_PRINT("thread_pool.size(): " << thread_pool.size());

        size_t num_columns = tensor.size(0);
        for (size_t i = 0; i < num_columns; i += thread_pool.size()) {
            size_t batch_end = std::min(i + thread_pool.size(), num_columns);
            for (size_t j = i; j < batch_end; ++j) {
                thread_pool.enqueue([this, &tensor, j]() {
                    DEBUG_PRINT("____________________________________________");
                    DEBUG_PRINT("thread_pool.size(): " << thread_pool.size());
                    DEBUG_PRINT("Initiated a context for index " << j);
                    DEBUG_PRINT("tensor.size(): " << tensor.size(0));
                    DEBUG_PRINT("tensor.dim(): " << tensor.dim());
                    
                    // DEBUG_PRINT("tensor.size(): " << tensor.size(1));
                    this->insert_context(tensor, j);
                });
            }
            // Wait for the current batch to complete before starting the next one
            // This assumes you have a way to wait for all tasks in the thread pool to complete
